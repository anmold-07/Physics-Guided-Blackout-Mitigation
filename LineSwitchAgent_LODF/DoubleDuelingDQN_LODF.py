...
class DoubleDuelingDQN(AgentWithConverter):
    def __init__(self,
        ...
        self.parse_observation_to_grid_time_zero(self.init_obs)                         # to init the system

        # loading the custom action space
        self.all_actions = []
        loaded_data = np.load(os.path.split(os.path.realpath(__file__))[0]+'/'+'track1_119_actions_numpy.npz') 
        actions = loaded_data['track1_119_actions_numpy']             # each action in this array is a vector action
        ...
        self.observation_size = 22 + 37 + 59 * 8 + 36   # same observation space as
        self.action_size = self.action_space.size()
        if cfg.VERBOSE:
            print ("self.obs_space.size_obs() = {}".format(self.obs_space.size_obs()))
    ...
    def makePTDF(self, baseMVA, bus, branch, slack):    
        ...
        nbuses = bus.shape[0]           # num buses
        nbranches = branch.shape[0]     # num branches
    ...
    def convert_obs(self, observation):
        tmp_list_vect = ['prod_p','load_p','p_or','a_or','p_ex','a_ex','rho','line_status',
                         'timestep_overflow','time_before_cooldown_line','time_before_cooldown_sub'] # 22 + 37 + 59*8 + 36
    ...
    def choose_best_action_by_simul(self, observation, tested_action):
        ...
        if len(tested_action) > 1:
            ...
            try:
                ...
                reward_idx = int(np.nanargmax(resulting_rewards))
    ...
    def findMonitoredLineandBus(self, obs):
        monitoredLineIndex = np.argmax(obs.rho)    
    ...
    def random_overflow_Action(self, can_be_reco, observation, removed_indices):

        # case-3: reconnection lines
        res1 = []
        if np.any(can_be_reco):
            res1 = [ self.action_space({"set_line_status": [(id_, +1)]}) for id_ in np.where(can_be_reco)[0] ]
        ...
        if len(finalScreeningTrueLineIDs)>0:
            res2 = [ self.action_space( {"set_line_status": [(id_, -1)]} ) for id_ in finalScreeningTrueLineIDs ]
        
        tested_actions = []
        if len(res1)>0 and len(res2)>0:
            print("CASE-3 : Feasible LODF and Reconnection ")
            tested_actions = res2 + res1 
    ...
    def train(self, env,
        ...
        while step < num_steps:
            ...
            removed_indices += 1        # index specific to the structure of the action space
            ...
            if max_rho_value < activateAgentRho:
                action_reco, reco_rwd = self.reco_line(observation, can_be_reco) # reconnect the best legal line
            ...
            # choose action/action selection based on e-greedy
                ...
                if np.random.rand(1) < self.epsilon: 
                    act = self.random_overflow_Action(can_be_reco, observation, removed_indices) # choosing a random legal grid2op action
